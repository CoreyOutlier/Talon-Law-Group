      ...
      <div ref={bed} className="absolute inset-0 will-change-transform">
        <div className="drift absolute inset-0">
          ...
          <img
            ...
            className="h-full w-full object-cover object-[70%_30%] md:object-[62%_42%]"
            decoding="async"
            fetchPriority="high"
          />
        </div>
        <div className="absolute inset-0 bg-gradient-to-r from-ink/85 via-ink/35 to-ink/5" />
        <div className="absolute inset-x-0 bottom-0 h-[58%] bg-gradient-to-t from-ink/95 via-ink/45 to-transparent" />
        <div className="absolute inset-x-0 top-0 h-44 bg-gradient-to-b from-ink/70 to-transparent" />
      </div>

      {/* copy */}
      <div ref={copy} className="shell relative flex h-full flex-col justify-end pb-[max(3rem,7vh)] pt-32 will-change-transform">
        <p className="hero-meta eyebrow mb-7">Order of Barristers · Trial practice since {site.founded}</p>
        <h1 className="display display-xl max-w-[12ch] text-[clamp(2.75rem,8.2vw,7.75rem)] text-paper">
          {LINES.map((l, i) => (
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-12 lg:items-end">
          <p className="hero-meta max-w-[44ch] text-[1.0625rem] leading-relaxed text-paper/75 text-pretty md:text-[1.1875rem] lg:col-span-6">
        ...
        <span className="hero-rule mt-10 block h-px w-full origin-left bg-paper/25" />
        <div className="hero-meta mt-5 flex flex-wrap items-center justify-between gap-4 font-display text-[10px] uppercase tracking-[0.26em] text-paper/55">
          <p>No fee unless we win · Answered 24/7 · Free consultation</p>
          <p className="hidden sm:block">{site.cities.join(" · ")}</p>
        </div>
      </div>
